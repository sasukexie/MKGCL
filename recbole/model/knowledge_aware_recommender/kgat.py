...
class KGAT(KnowledgeRecommender):
    ...
    def calculate_loss(self, interaction, batch_idx):
        ...
        pos_i_embeddings = self.projection_head_map(pos_embeddings, 1 - self.mode)
        reg_loss += self.algn_uniform_loss(batch_idx, u_embeddings, pos_i_embeddings)
    ...
    def projection_head_map(self, state, mode):
        """
        1.通过bn来实现embedding的打散。BN的引入参见: Bootstrap Your Own Latent A New Approach to Self-Supervised Learning
        2.通过不断切换bn的mode来达到与shuffle bn同等的效果。Shuffle BN 参见: Momentum Contrast for Unsupervised Visual Representation Learning
        """
    ...
    def algn_uniform_loss(self, batch_idx, x, y):
        align_loss_val = CalAlignAndUniformLoss.align_loss(x, y, alpha=self.config['align_alpha'])
        unif_loss_val = (CalAlignAndUniformLoss.uniform_loss(x, t=self.config['unif_t']) + CalAlignAndUniformLoss.uniform_loss(y, t=self.config['unif_t'])) / 2
        loss = align_loss_val * self.config['align_w'] + unif_loss_val * self.config['unif_w']
        # loss.detach()
        self.align_meter.update(align_loss_val, x.shape[0])
        self.unif_meter.update(unif_loss_val)
        self.loss_meter.update(loss, x.shape[0])

        if batch_idx % self.config['log_interval'] == 0:
            self.logger.info(f"{self.align_meter}\t{self.unif_meter}\t{self.loss_meter}")
        # loss.backward()
        return loss
    ...
    def update_attentive_A(self):
        ...
        for rel_idx in range(1, self.n_relations, 1):
            ...
            kg_score = self.generate_transE_score(self.all_hs[triple_index].cpu(), self.all_ts[triple_index].cpu(), rel_idx)
