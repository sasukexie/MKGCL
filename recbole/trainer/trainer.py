...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.running_flag = config['running_flag']
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            if self.config['model'] == 'MKGCL':
                if self.config['open_r'] and self.config['r_data_aug'] == 'r_s' and loss_func == self.model.calculate_kg_loss:
                    r_s = None
                    try:
                        r_s = train_data.dataset.r_s
                    except Exception as e:
                        pass
                    losses = loss_func(interaction, r_s)
                elif loss_func == self.model.calculate_loss:
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # read running_param
            try:
                fileObj = open("./zone/running_param.txt", 'r')
                text = fileObj.read()
                fileObj.close()
...
class MKGCLTrainer(Trainer):
    r"""MKGCLTrainer is designed for MKGCL, which is a knowledge-aware recommendation method.

    """

    def __init__(self, config, model):
        super(MKGCLTrainer, self).__init__(config, model)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        # MKGCL open_r
        if self.config['open_r'] and self.config['r_data_aug'] == 'r_s':
            h = train_data.dataset.head_entities
            r = train_data.dataset.relations
            t = train_data.dataset.tail_entities
            h_r_t = {}
            for i in range(len(r)):
                h_i = h[i]
                r_i = r[i]
                t_i = t[i]
                h_t = str(h_i) + '_' + str(t_i)
                if h_t not in h_r_t:
                    h_r_t[h_t] = set()
                h_r_t[h_t].add(r_i)

            r_set = set()
            for rs in h_r_t.values():
                r_set.add(str(rs).replace('{', '').replace('}', '').replace(' ', ''))

            # sort by count(r) reverse
            r_arr1 = sorted(r_set, key=lambda x: x.count(','), reverse=True)
            # r str -> list(int)
            r_arr = []
            r_len = 0
            for rs in r_arr1:
                arr = list(map(int, rs.split(',')))
                arr_len = len(arr)
                if arr_len <= 1:
                    break
                r_len += arr_len
            ...
            train_data.dataset.r_s = {'arr': r_arr, 'len': r_len}
